...
def convert_720p(source):
    ...
    cmd = ['/usr/bin/ffmpeg', '-y', '-i', source, '-s', 'hd720', '-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-strict', '-2', new_file_name]
    try:
...
def convert_480p(source):
    ...
    cmd = ['/usr/bin/ffmpeg', '-y', '-i', source, '-s', 'hd480', '-c:v', 'libx264', '-crf', '23', '-c:a', 'aac', '-strict', '-2', new_file_name]
    try:
        subprocess.run(cmd, check=True)
        print(f"Video erfolgreich konvertiert zu: {new_file_name}")
    except subprocess.CalledProcessError as e:
